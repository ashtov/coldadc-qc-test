...
# Defining main()
def main(GPort,SPI,SERIAL0,FIFO,NWORD, NSamples):
...
#    gen_set = srs_interface.getFreq()
#    srs_interface.setFreq(100000)
#    time.sleep(uniform(0, 500)/1000)
#    srs_interface.setFreq(gen_set)
#    time.sleep(0.1)
...
#        adcDataBlock=SPI.readbytes(NWORD)
    ...
    for i in range(len(dataSample)):
        firstBlock=dataSample[i][0:NWORD]
        
#        printFPGAfifo(firstBlock)

    #Deserialize ADC data
        deSerData.append (deSerialize(firstBlock) )     
        if (i%10) == 0:
...
#    idata=sys.argv[2]
    ...
    deSerialData=main(GPIO,spi0,ser,FPGA_FIFO_FULL,NWORDFIFO,NSamples)
