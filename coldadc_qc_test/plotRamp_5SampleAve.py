...
#    idata=sys.argv[2]
    ...
    for iLoop in range(0,NumSet):
        ...
        for jx in range (0,len(iadcVal)):
            x0[iLoop][jx]=float(jx)*SampRate  
            y0[iLoop][jx]=float(iadcVal[jx])
    ...
    if NoiseStudy == 0 :
        ...
        print(p0)
        # now do the fit
        fit, cov = curve_fit(my_sin, x0[0], y0[0], p0=p0)
        # we'll use this to plot our first estimate. 
        data_first_guess = my_sin(x0[0], *p0)

        # recreate the fitted curve using the optimized parameters
        #data_fit = my_sin(x0, *fit[0])
        data_fit = my_sin(x0[0], *fit)
        print(p0)
        # residual (data - fit)
        residual = y0[0] - data_fit
        print("Max, min residuals = ",residual.max(),residual.min())
        print("2*amplitude from fit = ",2*fit[1])
        ...
        plt.title('Sinusoidal Input (147KHz; Vp-p 1.4V) ')

        ax1.plot(x0[0],y0[0],'.',label='ADC Output')
        ax1.plot(x0[0],data_fit, label='Fit')
        ...
        histResults=plt.hist(y0[0],bins=21, align='mid')
        #histResults=plt.hist(y0,bins=30)
        
        # now do the fit
        mean,std = norm.fit(y0[0])
        xmin,xmax = plt.xlim()
    ...
    fft.AnalyzeDynamicADC(y0[0],2048,16,2,window)
    plt.show()
