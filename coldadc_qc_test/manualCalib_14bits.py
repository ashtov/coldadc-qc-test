...
def computeAvgADC(GPort,SPI0,SERIAL0,FIFO_FULL,NWORD,numset,oFile):
    ...
    for iLoop in range(0,numset):
        ...
        deSerialData14 = [] 
        for currentCode in deSerialData:
            deSerialData14.append(int(np.floor(currentCode/4))*4)
        #printADCdata(deSerialData14)

        
        jCnt=0
        for iCnt in range(0,len(deSerialData14)):
        #for iCnt in range(0,32):
            if jCnt < 8 :
                AvgADC[0] = ((AvgADC[0]*iCntADC0)+(deSerialData14[iCnt]))/(iCntADC0 + 1.0)
                jCnt += 1
                iCntADC0 += 1
                ...
                AvgADC[1] = ((AvgADC[1]*iCntADC1)+(deSerialData14[iCnt]))/(iCntADC1 + 1.0)
                jCnt += 1
                iCntADC1 += 1
