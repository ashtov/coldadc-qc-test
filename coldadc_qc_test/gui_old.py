...
#    vinit_vals_rt = ['0xce', '0x2b', '0x7d', '0x5c']  # VDDA = 2.4V
    vinit_vals_rt = ['0xdc', '0x2f', '0x85', '0x62']  # VDDA = 2.4V
...
#        oscommand=("/home/dune/ColdADC/coldadc_qc_test/stability_study_Diff_Multi.sh {}".format(self.dirsave))
        ...
        rawtime = time.time()
        oscommand=('{}/qc_dnl_inl.py'.format(self.soft_dir))        
        os.system(oscommand)
        self.status_label.config(text="INL and DNL DONE")
        print(str(time.process_time() - timestamp) + " seconds to run linearity")
        print(str(time.time() - rawtime) + " raw seconds to run linearity")
...
#        if (abs(float(self.read_vdda('v')))<0.05):
...
##        dnlstat = calc_dnl_inl(self.dirsave, filename)
        os.system("{}/qc_fft_enob.py {} {} >> {}".format(self.soft_dir,self.dirsave, filename, repFilename))
#        if(self.temp_val.get()==2 or self.temp_val.get()==1):
        if(self.temp_val.get()==2):        

            self.sel_se_diff('SE')
            os.system("{}/qc_dnl_inl.py {} {} >> {}".format(self.soft_dir,self.dirsave, filename+'_SE', repFilename))

            self.sel_se_diff('DIFF')
            os.system("{}/qc_dnl_inl.py {} {} >> {}".format(self.soft_dir,self.dirsave, filename+'_DIFF', repFilename))
...
############ External VDDA Setup Methods #############################
    ...
    def vdda_onoff(self, state=None):
        ...
        print("Channel " + str(self.vdda_ps_chan))
    ...
    def read_vdda(self, quant):
        ...
        # ps_param = self.ps_interface.measure_params(self.vdda_ps_chan)

        # print(quant, self.vdda_ps_chan, ps_param)

        # if (quant == 'v'):
        #     self.test_vdda.config(text=("Ext VDDA: {} V".format(ps_param[0])))
        #     return ps_param[0]
        # elif (quant == 'p'):
        #     return ps_param[2]
...
############# Various GUI Control Methods ############################
    ...
    def ldo_off(self):
        ...
        self.readtest_button["state"] = "disable"
        self.chantest_button["state"] = "disable"
        self.ch16test_button["state"] = "disable"
        self.noistest_button["state"] = "disable"
        self.linrtest_button["state"] = "disable"
        self.readreg_button["state"] = "disable"
        self.readreg_button["state"] = "disable"
        self.reset_button["state"] = "disable"
        self.setvdda_button["state"] = "disable"
