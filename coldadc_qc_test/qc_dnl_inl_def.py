...
#    GPIO.output(UART_SEL, GPIO.HIGH)
    ...
    #NumSet=240   #4M samples for ADC test mode
    #NumSet=480  #1M samples for single channel read
    NumSet=1200  #2M samples for single channel read
    #NumSet=1
...
#        time.sleep(uniform(0, 10)/1000000)
    ...
    for ich in range (0, NChan):
        code[ich], dnl[ich], inlNorm[ich] = calc_linearity(y0[ich])
