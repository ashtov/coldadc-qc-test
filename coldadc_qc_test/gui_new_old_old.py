...
from qc_noise import calc_noise
from qc_fft_enob import calc_fft
...
#        self.avgnoise = calc_noise()
    ...
    def run_linearity(self):
        self.gen_onoff('ON')
        sleep (2)
        self.status_label.config(text="Running INL and DNL")
#        self.dirsave = self.data_base_dir+"/coldadc_"+self.asic_entries[0].get()
#        check_folder = os.path.isdir(self.dirsave)
#        if not check_folder: os.makedirs(self.dirsave)
#        oscommand=("/home/dune/ColdADC/coldadc_qc_test/stability_study_Diff_Multi.sh {}".format(self.dirsave))
        oscommand=('{}/qc_dnl_inl_new.py'.format(self.soft_dir))
        os.system(oscommand)
...
#############  Measure DNL/INL at LN2
        if(self.temp_val.get()==2 or self.warmonly.get()==1):        
            ...
            gport = readADC_NSamp_new.initGPIOpoll(self.FPGA_FIFO_FULL)
            import pandas as pd
            ############dd#  Differential Mode
            rawLinDataDiff = readADC_NSamp_new.readNSamp(gport,spi,self.FPGA_FIFO_FULL,self.NWORDFIFO,self.DNLSamples)
            Name=[]
            for i in range(len(rawLinDataDiff)):
                Name.append(i)
            name_dict={
                'Name':Name,
                'rawLinDataDiff':rawLinDataDiff
                }
            df=pd.DataFrame(name_dict)
            df.to_csv("{}/rawLinDataDiff.csv".format(self.dirsave))
            
            
            #############  Single-Ended Mode
            self.sel_se_diff('SE')
      
            rawLinDataSE = readADC_NSamp_new.readNSamp(gport,spi,self.FPGA_FIFO_FULL,self.NWORDFIFO,self.DNLSamples)
            Name=[]
            for i in range(len(rawLinDataSE)):
                Name.append(i)
            name_dict={
                'Name':Name,
                'rawLinDataSE':rawLinDataSE
                }
            df=pd.DataFrame(name_dict)
            df.to_csv("{}/rawLinDataSE.csv".format(self.dirsave))

            spi.close()
            gport.cleanup

            self.sel_se_diff('DIFF')
#            self.status_label.config(text="DNL/INL Test DONE")        
...
##Skip the calculation of INL/DNL
        """
        deserLinDataDiff = []
        deserLinDataSE = []
        if(self.temp_val.get()==2 or self.warmonly.get()==1):
            ...
            for i in range(len(rawLinDataDiff)):
                firstBlock=rawLinDataDiff[i][0:self.NWORDFIFO]
            ...
            qc_dnl_inl_new.calc_dnl_inl(self.dirsave, filename+'_DIFF', self.DNLSamples, deserLinDataDiff)
            del deserLinDataDiff

            for i in range(len(rawLinDataSE)):
                firstBlock=rawLinDataSE[i][0:self.NWORDFIFO]
            ...
            qc_dnl_inl_new.calc_dnl_inl(self.dirsave, filename+'_SE', self.DNLSamples, deserLinDataSE)
            del deserLinDataSE
            """
        messagebox.showinfo(title="Information", message="ColdADC QC Test Has Finished.(INL/DNL not calculated.)"+"NWORDFIFO="+str(self.NWORDFIFO)+"DNLSamples="+str(self.DNLSamples)+"filename="+str(filename)+"dirsave="+str(self.dirsave))
        self.ldo_off()
